...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ফাংশন ডিক্লারেশন আগে (hoisting এর জন্য)
    ...
    // ফিক্সড: ভিডিও লোড করার ফাংশন (CORS সমস্যা সমাধান করা হয়েছে)
    function loadVideoToModal(videoId, headline) {
        const iframe = document.getElementById('ytVideo');
        const messageDiv = document.querySelector('.error-message');
        const videoPlayer = document.getElementById('modalVideoPlayer');
        
        // চেক করা এটি একটি বৈধ YouTube URL কিনা
        const isValidYouTube = videoId && (
            videoId.includes('youtube.com/embed/') || 
            videoId.includes('www.youtube.com/embed/') ||
            (videoId.startsWith('http') && (videoId.includes('youtu.be/') || videoId.includes('youtube.com')))
        );
        
        // লোকাল ভিডিও ফাইল চেক করা (.mp4)
        const isLocalVideo = videoId && (videoId.endsWith('.mp4') || videoId.includes('.mp4'));
        ...
        if (isValidYouTube) {
            ...
            iframe.onerror = function() {
                messageDiv.innerHTML = `
                    <div class="internetcanectnone">
                        ...
                        <div class="videoheadname">${headline || 'ভিডিও'}</div>
                        <div class="fastifo">ভিডিও লোড করা যায়নি।</div>
                        <div class="secendifo">ইন্টারনেট কানেকশন চেক করুন অথবা পরে আবার চেষ্টা করুন।</div>
                    </div>`;
                messageDiv.classList.remove('loading');
            ...
            // লোকাল ভিডিও ফাইলের জন্য (যদি থাকে)
            videoPlayer.innerHTML = `<video controls style="width:100%; height:480px;"><source src="${videoId}" type="video/mp4">আপনার ব্রাউজার ভিডিও সাপোর্ট করে না।</video>`;
            messageDiv.style.display = 'none';
            
        } else {
            // ভিডিও নেই (প্লেসহোল্ডার URL)
            messageDiv.innerHTML = `
                <div class="internetcanectnone">
                    <div class="neticon"><i class="fas fa-video-slash"></i></div>
                    <div class="videoheadname">${headline || 'ভিডিও'}</div>
                    <div class="fastifo">ভিডিওটি এখনো যোগ করা হয়নি।</div>
                    <div class="secendifo">শীঘ্রই এখানে ভিডিও যোগ করা হবে।</div>
                </div>`;
            messageDiv.classList.remove('loading');
            messageDiv.style.display = 'flex';
            iframe.style.display = 'none';
    ...
    function showLoadingMessage(errormasege) {
        errormasege.classList.add('loading');
        errormasege.style.display = 'flex';
    ...
    function updateModalContent(videoId, headline, description, headteglin, catagoriname) {
        ...
        showLoadingMessage(errormasege);
        
        // ভিডিও লোড করা
        loadVideoToModal(videoId, headline);
    };
    ...
    function updateMainGallery(activeVideoId) {
        const allVideoItems = document.querySelectorAll('.video-container');
    ...
    document.querySelector('.close').addEventListener('click', function () {
        ...
        // ভিডিও বন্ধ করা এবং iframe ক্লিয়ার করা
        const videoPlayer = document.getElementById('modalVideoPlayer');
        const iframe = document.getElementById('ytVideo');
        if (iframe) {
            iframe.src = ''; // ভিডিও স্টপ করা
        }
        videoPlayer.innerHTML = `<div id="modalVideoPlayer"></div>`;
